...
up_genes <- gbm[gbm$log2fold_change > 1 & gbm$pvalues < 0.15, ]
rownames(up_genes) # list of up-regulated genes
down_genes <- gbm[gbm$log2fold_change < -1 & gbm$pvalues < 0.15, ]
rownames(down_genes) # list of down-regulated genes 


# read enrichment.csv downloaded from ShinyGO after functional enrichment analysis using all available gene sets
# the enrichment analysis was done only on down-regulated genes and sorted by fold enrichment
...
# visualizing the top5 down-regulated pathways using a dot/bubble plot
library(ggplot2)
# factorising Pathway column to ensure that ggplot2 uses the order that I want for the plot
top5$Pathway <- factor(top5$Pathway, levels = rev(top5$Pathway)) 

# dot/bubble plot code
